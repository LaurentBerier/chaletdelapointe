    ...
    <div className="min-h-[100dvh] flex flex-col bg-background text-foreground font-sans">
      ...
      >
        <div className="container mx-auto px-6 md:px-12 flex items-center justify-between">
          <Link
            href="/"
            className={`text-2xl font-serif tracking-tight font-medium transition-colors ${
              location === "/" && !isScrolled ? "text-white" : "text-foreground"
            }`}
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={`text-sm tracking-wide uppercase transition-colors hover:opacity-100 ${
                  location === link.href
                    ? location === "/" && !isScrolled ? "text-white opacity-100 font-medium" : "text-primary opacity-100 font-medium"
                    : location === "/" && !isScrolled ? "text-white/70 hover:text-white" : "text-foreground/70 hover:text-foreground"
                }`}
          ...
          <button
            className={`md:hidden p-2 -mr-2 transition-colors ${
              location === "/" && !isScrolled && !mobileMenuOpen ? "text-white" : "text-foreground"
            }`}
      ...
      <footer className="bg-primary text-primary-foreground py-12 md:py-16 mt-auto">
        <div className="container mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-3 gap-12">
          ...
          <div>
            <h4 className="font-medium mb-4 text-sm tracking-wider uppercase">Liens</h4>
